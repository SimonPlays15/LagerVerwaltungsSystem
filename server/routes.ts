...
import {DependencyConflictError, storage} from "./storage";
import {changePassword, isAuthenticated, loginUser, registerUser, setupAuth,} from "./localAuth";
import {changePasswordSchema, insertCustomerSchema, loginUserSchema, registerUserSchema,} from "../shared/schema";
import {
    insertArticleSchema,
...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    app.get("/api/customers/active", isAuthenticated, async (req, res) => {
        try {
    ...
    app.put("/apu/customers/:id", isAuthenticated, async (req: any, res) => {
        try {
            const user = await storage.getUser(req.user.id);
            if (!user || (user.role !== "admin" && user.role !== "projektleiter")) {
                return res.status(403).json({message: "Insufficient permissions"});
